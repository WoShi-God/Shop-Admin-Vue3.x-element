            ...
            <div class="flex-1">
              ...
              <Transition name="comment" mode="out-in">
                ...
                <div v-if="row.textareaEdit">
                  <el-input v-model="textarea" type="textarea" :rows="2" placeholder="请输入回复评论内容"></el-input>
                  <div class="py-2">
                    <el-button type="primary" size="small" @click="review(row)">回复评论</el-button>
                    <el-button size="small" class="ml-2" @click="row.textareaEdit = false">取消评论</el-button>
                  </div>
                </div>
                <!-- 客服回复 -->
                <div v-else>
                  <div v-for="(item, index) in row.extra" :key="index" class="mt-3 p-3 bg-gray-100 rounded">
                    <h6 class="flex font-bold">
                      商品客服小姐姐
                      <el-button type="info" size="small" class="ml-auto" @click="openTextarea(row, item.data)">修改评论
                      </el-button>
                    </h6>
                    <p>{{ item.data }}</p>
                  </div>
